...
export const AUTHORING_COLUMNS = {
  ...
  passwords: ["comment", "content", "password", "notes"],
  ...
  talk_flow: ["comment", "talk", "from", "cond", "intent", "criteria", "example", "match", "next", "mode", "set", "notes"],
...
const requiredColumns = {
  ...
  talk_people: ["id", "name", "role", "avatar"], talk_flow: ["talk", "from", "cond", "intent", "criteria", "example", "match", "next", "mode"],
...
const requiredCells = {
  ...
  talk_flow: ["talk", "from", "next"], assistant_messages: ["id", "surface", "body"], attachments: ["id", "type"],
...
export function compileScenarioAuthoring(workbook) {
  ...
  for (const row of rows("photo_items")) {
    ...
    const record = { title: text(row, "title"), ...(image ? { imageUrl: asset(image, "image") } : {}),
      ...(video ? { mediaKind: "video", videoUrl: asset(video, "video") } : audio ? { mediaKind: "still_video", audioUrl: asset(audio, "audio") } : {}),
  ...
  for (const row of rows("call_items")) contents.push(item(row, "phone", {
    ...
    ...(text(row, "audio") ? { audioUrl: asset(text(row, "audio"), "audio") } : {}),
  ...
  for (const row of rows("radio_items")) {
    ...
    if (audio.length === 1 && !audio[0].startsWith("gen_audio:")) record.audioUrl = asset(audio[0], "audio");
    else if (audio.length) record.audioSegments = audio.map((id) => id.startsWith("gen_audio:")
      ? { kind: "generated", genAudioId: id.slice(10) } : { kind: "audio", audioUrl: asset(id, "audio") });
  ...
  for (const row of rows("passwords")) {
    ...
    if (!password.trim() || password.length > 80) throw new Error(`passwords.${id}: passwordは空でない80文字以内の値が必要です。`);
  ...
  const source = {
    ...
    project: { id: value("project.id"), name: value("project.name"), osName: value("device.os_name"), assistantName: value("search_agent.name"),
      ...
      lockScreen: { method, ...(method === "fixed-pin" ? { pin: value("device.lock_pin") } : {}) } },
    ...
    incomingCalls: rows("incoming_calls").map((row) => ({ id: text(row, "id"), name: row.name ?? "", cond: text(row, "cond"), ...(text(row, "audio") ? { audioUrl: asset(text(row, "audio"), "audio") } : {}), ...(text(row, "transcript") ? { transcript: jsonCell(row, "transcript") } : {}) })),
