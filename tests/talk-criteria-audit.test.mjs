...
const rule = { __rowNumber: 2, talk: "fixture", from: "question", intent: "分岐", type: "ai", criteria: "プレイヤーが具体的な質問をしている。", match: "", example: "質問です", mode: "" };

test("criteriaの任意lintは場面説明・入力条件・写真・game overの書き分けを診断する", () => {
  const { findings, summary } = auditTalkCriteria([
    { ...rule, __rowNumber: 2, intent: "", type: "default", criteria: "" },
    { ...rule, __rowNumber: 3, type: "ai", criteria: "次へ進める" },
    { ...rule, __rowNumber: 4, type: "ai", criteria: "写真", example: "photo:sample" },
    { ...rule, __rowNumber: 5, type: "ai", criteria: "相手を困らせる", mode: "game_over" },
    { ...rule, __rowNumber: 6, type: "match", criteria: "/unterminated" }
  ]);
...
test("正規表現の条件を自然文lintへ混ぜず、既定文脈や長さの診断を保持する", () => {
  const { findings } = auditTalkCriteria([
    { ...rule, type: "match", criteria: "/^次へ$/u", mode: "game_over" },
    { ...rule, __rowNumber: 3, intent: "", type: "default", criteria: "説明" },
    { ...rule, __rowNumber: 4, intent: "", type: "default", criteria: "通常候補に一致しない場合に待つ。".repeat(30) },
...
test("正規表現だけの場面へLLM用のdefault文脈を要求しない", () => {
  const fallback = { ...rule, intent: "", type: "default", criteria: "" };
  assert.deepEqual(auditTalkCriteria([
    fallback,
    { ...rule, type: "match", criteria: "/^確認$/u" }
  ]).findings, []);
  assert.ok(auditTalkCriteria([
    fallback,
    { ...rule, from: "*", type: "ai", criteria: "プレイヤーが明示的な質問をしている" }
