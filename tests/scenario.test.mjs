...
test("デモシナリオは検索アプリを作らず、修復対象を保持する", () => {
  ...
  assert.equal(scenario.worker.apps.find((app) => app.id === "chat")?.cond, "clue_reported");
});
...
test("会話ブロックは複数発話・添付・遅延・出典を保持する", () => {
  ...
  const foundRule = talk?.rules.find((rule) => rule.intent === "手がかり発見");
  ...
  assert.equal(messages[1]?.attachmentId, "orange_mark_image");
  ...
  assert.equal(scenario.deviceState.notes.some((note) => note.title === "鍵付き添付の中身"), false);
...
test("着信定義と予約イベント用hookを生成する", () => {
  const scenario = loadAndValidateScenario();
  assert.equal(scenario.worker.incomingCalls.find((call) => call.id === "demo_call")?.name, "持ち主候補");
...
test("条件付き表示と検索応答を公開シナリオへ生成する", () => {
  ...
  assert.equal(scenario.worker.talks.find((talk) => talk.id === "lobby")?.cond, "clue_reported");
  assert.equal(scenario.worker.notifications.find((item) => item.id === "welcome")?.cond, "!old_note_opened");
  assert.equal(scenario.worker.assistantMessages.find((item) => item.id === "home_hint")?.cond, "!clue_reported");
  assert.equal(scenario.worker.searchResponses.find((item) => item.id === "hint")?.suppressResults, true);
  assert.equal(scenario.worker.chatAuthGate?.cond, "clue_reported && !chat_auth_verified");
...
test("作品Stageには明示した状態変数だけを公開する", () => {
  workerScenario.publicStateVariables.push("clue_reported");
  try {
    const state = createInitialPlayerState();
    assert.deepEqual(publicPlayerState(state, 1).projectState, { clue_reported: false });
    state.stateValues.clue_reported = true;
    assert.deepEqual(publicPlayerState(state, 2).projectState, { clue_reported: true });
...
test("未到達の本文とtalkは初期応答へ含めず、利用可能になった時だけ差分で返す", () => {
  ...
  assert.doesNotMatch(serialized, /オレンジ色の印を見つけたら/u);
  ...
  reachedState.stateValues.clue_reported = true;
