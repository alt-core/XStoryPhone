// scenario:build によりhooks.tsvから生成。直接編集しないでください。
...
export const scenarioHookHandlers: ScenarioHookHandlerRegistry<ScenarioHookId, ProjectScenarioHookContext> = {
  "mark_session_started": (context, event) => {
  ...
  "mark_old_note_opened": (context, event) => {
    const { state, incoming, app, content, talk, todo, schedule, form, genAudio, llm, effect, effectSequence } = context;
  ...
  "mark_rainy_window_opened": (context, event) => {
    const { state, incoming, app, content, talk, todo, schedule, form, genAudio, llm, effect, effectSequence } = context;
  ...
  "deliver_clue_attachments": (context, event) => {
    const { state, incoming, app, content, talk, todo, schedule, form, genAudio, llm, effect, effectSequence } = context;
  ...
  "mark_sealed_note_unlocked": (context, event) => {
    const { state, incoming, app, content, talk, todo, schedule, form, genAudio, llm, effect, effectSequence } = context;
  ...
  "schedule_demo_call": (context, event) => {
    const { state, incoming, app, content, talk, todo, schedule, form, genAudio, llm, effect, effectSequence } = context;
    context.schedule.after("show_demo_call", 1_000, {}, "demo_call_once");
  },
  "show_demo_call": (context, event) => {
    const { state, incoming, app, content, talk, todo, schedule, form, genAudio, llm, effect, effectSequence } = context;
    context.incoming.start("demo_call");
  },
  "deliver_demo_delayed_message": (context, event) => {
    const { state, incoming, app, content, talk, todo, schedule, form, genAudio, llm, effect, effectSequence } = context;
  ...
  "mark_demo_call_completed": (context, event) => {
    const { state, incoming, app, content, talk, todo, schedule, form, genAudio, llm, effect, effectSequence } = context;
  ...
  "complete_demo_todo": (context, event) => {
    const { state, incoming, app, content, talk, todo, schedule, form, genAudio, llm, effect, effectSequence } = context;
