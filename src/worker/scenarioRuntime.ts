...
import { mediaAttachmentIds, resolveMediaRecord, usesStandardMedia } from "../shared/scenarioMedia.ts";
...
import type { PublicGeneratedAudioState, ScenarioContent, ScenarioDeviceTalk, ScenarioMessageAttachment, ScenarioMessageSegment, ScenarioSearchAgentTalk, ScenarioTalk, TalkOutputStep, StoredTalkMessage } from "../shared/scenario.ts";
...
export function createScenarioRuntime(workerScenario: WorkerScenario) {
  const talkEvents = createTalkEventsRuntime({ messageTemplatesForBlock, talkBlockIdForRepeatDisplay });
  const {
  ...
  function rebuildIndexes() {
    ...
    return {
      ...
      attachmentsById,
      projectAppById,
      attachmentIdByPublicId,
      incomingCallIdByPublicId,
      talkHistoryRepairs,
      talkHistoryRepairByBlockId,
      talkHistoryRepairByContentId,
      talkHistoryRepairByPublicContentId,
      albumPhotoIdsByAttachmentId,
      albumAttachmentIdsByPhotoId,
    };
  }
  function isSearchAgentTalk(talk: ScenarioTalk): talk is ScenarioSearchAgentTalk {
    return talk.kind === "search_agent";
  }
  function isDeviceTalk(talk: ScenarioTalk): talk is ScenarioDeviceTalk {
    return talk.kind !== "search_agent";
  }
  let {
    peopleById,
  ...
  function useScenario(next: WorkerScenario) {
    // 既に選択・保存したviewを上書きせず、実行中の参照だけ切り替える。
    workerScenario = next;
    ({ peopleById, blocksById, attachmentsById, projectAppById, attachmentIdByPublicId, incomingCallIdByPublicId, talkHistoryRepairs, talkHistoryRepairByBlockId, talkHistoryRepairByContentId, talkHistoryRepairByPublicContentId, albumPhotoIdsByAttachmentId, albumAttachmentIdsByPhotoId } = rebuildIndexes());
  }
  function publicAttachmentId(internalId: string) {
    return workerScenario.publicIds.attachment[internalId] ?? internalId;
  }
  function internalAttachmentId(publicId: string) {
    return attachmentIdByPublicId.get(publicId) ?? "";
  }
  function internalIncomingCallId(publicId: string) {
    return incomingCallIdByPublicId.get(publicId) ?? "";
  }
  function internalFormId(publicId: string) {
  ...
  function albumPhotoIdsForMediaAttachment(attachment: unknown) {
    ...
    if (!["image", "audio", "video"].includes(String(record.kind)) || typeof record.attachmentId !== "string")
      return [];
    return albumPhotoIdsByAttachmentId.get(record.attachmentId) ?? [];
  }
  function messageTemplatesForBlock(blockId: string) {
    ...
    return (block?.messages ?? []).map((message) => {
      ...
      return {
        ...
        senderRole: block?.unavailable ? message.initialRole ?? "npc" : person?.role ?? "npc",
        ...(person?.avatar ? { avatarUrl: person.avatar } : {}),
        attachment: resolveTalkAttachment(message.attachmentId)
      };
    });
  }
  function talkBlockIdForRepeatDisplay(blockId: string, previousDisplayCount: number) {
    if (previousDisplayCount <= 0)
      return blockId;
  ...
  function requireBlockTemplates(blockId: string, values: Readonly<Record<string, unknown>>) {
    for (const message of blocksById.get(blockId)?.messages ?? []) {
      const attachment = attachmentsById.get(message.attachmentId);
      if (message.attachmentId && !attachment)
        throw new Error(`出力する添付のpartが未取得です: ${message.attachmentId}`);
      if (attachment?.poster && !attachmentsById.has(attachment.poster))
        throw new Error(`添付posterのpartが未取得です: ${attachment.poster}`);
    }
    for (const template of messageTemplatesForBlock(blockId)) {
      requireTemplateValues(template.body, values);
      for (const segment of template.segments ?? [])
        if (segment.kind === "text")
          requireTemplateValues(segment.text, values);
      for (const reply of template.quickReplies ?? [])
        requireTemplateValues(reply, values);
    }
  }
  function renderedSegments(segments: readonly ScenarioMessageSegment[] | undefined, env: Record<string, string>, messageId: string) {
  ...
  function messagesForInitialTalkBlocks(input: {
    ...
    const templateEnv: Record<string, string> = Object.fromEntries(Object.entries(input.formatEnv)
  ...
  function messagesForTalkBlocks(input: {
    talk: ScenarioDeviceTalk;
    blockIds: readonly string[];
    ...
    for (const [blockIndex, blockId] of input.blockIds.entries()) {
      ...
      requireBlockTemplates(displayBlockId, input.formatEnv);
      if (input.useRepeat !== false)
        blockDisplayCounts[blockId] = previousCount + 1;
  ...
  function applyTalkInputAction(state: TalkInputState, action: TalkInputAction, latestDisplaySeq: number) {
    if (action === "show") {
  ...
  function messagesForTalkOutputSteps(input: {
    talk: ScenarioDeviceTalk;
  ...
  async function initialTalkTurnKey(playerId: string, talkId: string, fromId: string) {
    return `turn_${(await sha256Hex(`turn:v1:${playerId}:${talkId}:${fromId}:initial`)).slice(0, 24)}`;
  }
  async function nextTalkTurnKey(playerId: string, talkId: string, currentTurnKey: string, nextFromId: string) {
    return `turn_${(await sha256Hex(`turn:v1:${playerId}:${talkId}:${currentTurnKey}:${nextFromId}`)).slice(0, 24)}`;
  }
  function searchAgentTimelineForOutputs(input: {
    ...
    for (const [outputIndex, output] of input.outputs.entries()) {
      if (output.kind === "block" && (!blocksById.has(output.blockId) || blocksById.get(output.blockId)?.unavailable))
        throw new Error(`出力blockのpartが未取得です: ${output.blockId}`);
      if (output.kind === "block")
        requireBlockTemplates(talkBlockIdForRepeatDisplay(output.blockId, blockDisplayCounts[output.blockId] ?? 0), input.formatEnv);
      if (output.kind === "input") {
        applyTalkInputAction(inputState, output.action, latestDisplaySeq);
        continue;
      }
      const deliveredAt = new Date(Date.parse(input.baseSentAt) + displayOutputIndex * 1000).toISOString();
      ...
      if (output.kind === "block") {
        ...
        if (rendered.events.length)
          latestDisplaySeq = rendered.lastSeq;
        continue;
      }
  ...
  async function initializeSearchAgentTalkState(talk: ScenarioSearchAgentTalk, playerId: string, state: StoredPlayerState, at = new Date().toISOString()) {
  ...
  async function talkTurnHash(playerId: string, kind: "sms" | "chat", talkId: string, turnKey: string) {
    return (await sha256Hex(`${playerId}:${kind}:${talkId}:${turnKey}`)).slice(0, 32);
  }
  async function scenarioMessageBlockId(playerId: string, kind: "sms" | "chat", talkId: string, blockId: string) {
  ...
  function historySlotsForTalk(talk: ScenarioDeviceTalk, state: StoredPlayerState) {
    return (state.talks[talk.id]?.historySlots ?? []).filter((slot) => (talkHistoryRepairByPublicContentId.get(slot.repairId)?.talk.id === talk.id));
  }
  function talkHistoryRevision(talk: ScenarioDeviceTalk, state: StoredPlayerState) {
    return historySlotsForTalk(talk, state).filter((slot) => (state.repairedContentIds.includes(talkHistoryRepairByPublicContentId.get(slot.repairId)?.content.id ?? ""))).length;
  }
  ...
  function publicTalkLastMessageSeq(talk: ScenarioDeviceTalk, state: StoredPlayerState) {
    ...
    if (!stored)
      return 0;
    if (!stored.historySlots.length)
      return stored.lastMessageSeq;
    if (stored.lastMessageSeq > stored.initialHistoryLastSeq)
      return stored.lastMessageSeq;
  ...
  function talkInputBoundarySeq(talk: ScenarioTalk, state: StoredPlayerState) {
    return isSearchAgentTalk(talk)
      ? state.talks[talk.id]?.lastMessageSeq ?? 0
      : publicTalkLastMessageSeq(talk, state);
  }
  function initializeTalkState(talk: ScenarioDeviceTalk, turnKey: string, formatEnv = workerScenario.stateVariables, repairedContentIds: readonly string[] = []) {
    const initialFormatEnv = Object.assign({}, ...talk.startBlocks.map((blockId) => (formatEnvForMessageBlockFromState(blockId, formatEnv) ?? {})));
  ...
  function initialTalkMessagesForState(talk: ScenarioDeviceTalk, formatEnv: Record<string, unknown>, repairedContentIds: readonly string[]) {
    ...
    const visibleMessages = rendered.messages.flatMap((message) => {
      ...
      if (message.scenarioBlockId && blocksById.get(message.scenarioBlockId)?.unavailable
        && (!repair || repairedContentIds.includes(repair.content.id)))
        throw new Error(`初期履歴の本文partが未取得です: ${message.scenarioBlockId}`);
      if (repair)
        return repairedContentIds.includes(repair.content.id) ? [message] : [];
      const { scenarioBlockId: _scenarioBlockId, ...plainMessage } = message;
      return [plainMessage];
    });
    return { rendered, visibleMessages };
  }
  function restoredTalkHistoryMessages(state: StoredPlayerState, contentId: string) {
    const repair = talkHistoryRepairByContentId.get(contentId);
    if (repair?.content.unavailable || (repair && blocksById.get(repair.blockId)?.unavailable))
      throw new Error(`復元する履歴のpartが未取得です: ${contentId}`);
    ...
    if (!slot)
      return { ok: false as const, error: "history_not_initialized" as const };
    const rendered = messagesForInitialTalkBlocks({
      talk: repair.talk,
  ...
  function synchronizeInitialTalkLastOtherMessageId(state: StoredPlayerState, talkId: string) {
    const talk = talkByInternalId(talkId);
    const stored = state.talks[talkId];
    if (!talk || !isDeviceTalk(talk) || !stored || stored.lastMessageSeq > stored.initialHistoryLastSeq)
      return state;
    const latestOther = [...initialTalkMessagesForState(talk, stored.initialFormatEnv, state.repairedContentIds).visibleMessages].reverse().find((message) => message.sender === "other");
    if (latestOther)
      stored.lastOtherMessageId = latestOther.id;
    return state;
  }
  function talkHistoryRepairAvailable(state: StoredPlayerState, contentId: string) {
    const repair = talkHistoryRepairByContentId.get(contentId);
    if (!repair)
      return true;
  ...
  function createInitialPlayerState(): StoredPlayerState {
    return {
      loadedPartIds: ["base"],
      repairedContentIds: [],
      repairedAppIds: [],
  ...
  function reconcileSearchAgentMessageLinks(state: StoredPlayerState, talkId: string) {
    ...
    const baseMessages = workerScenario.playerMode === "server"
      ? Object.entries(counts).flatMap(([baseBlockId, displayCount]) => (Number.isInteger(displayCount) && displayCount > 0
        ? messageTemplatesForBlock(baseBlockId).map((message) => ({ segments: message.segments }))
        : []))
      : [];
    const reconciledState = {
      ...state,
      revealedMessageLinks: state.revealedMessageLinks.filter((link) => (link.talkId !== talkId || !link.id.startsWith("search-link_") || currentLinkIds.has(link.id)))
    };
    return revealTalkMessages(reconciledState, talkId, baseMessages);
  }
  async function reconcileScenarioState(state: StoredPlayerState, playerId: string) {
    ...
    for (const talk of workerScenario.talks) {
      if (talks[talk.id] || !talkAvailable(talk, nextState))
        continue;
      if (isSearchAgentTalk(talk)) {
        ...
        nextState = revealTalkMessages(nextState, talk.id, initial.messages.filter((message) => message.type === "message"));
        continue;
      }
      const initial = initializeTalkState(talk, await initialTalkTurnKey(playerId, talk.id, talk.initialFrom), effectiveStateValues(workerScenario.stateVariables, stateValues), nextState.repairedContentIds);
      talks[talk.id] = initial.state;
  ...
  function validatePartState(state: StoredPlayerState) {
    for (const content of workerScenario.contents) {
      if (usesStandardMedia(content.appId) && mediaAttachmentIds(content.record).length && appAvailable(content.appId, state) && conditionMet(content.cond, state)
        ...
        resolveMediaRecord(content.record,workerScenario.attachments);
      }
    }
    if (state.incomingCallId && !state.completedIncomingCallIds.includes(state.incomingCallId)) {
      const call=workerScenario.incomingCalls.find(call => call.id === state.incomingCallId);
      if (call) resolveMediaRecord(call,workerScenario.attachments);
    }
  }
  function validateTalkContinuation(talkId: string, expectedFrom: string, state: StoredPlayerState) {
    const talk = talkByInternalId(talkId);
    const stored = state.talks[talkId];
    // hookによる意図した終話・移動、明示disableを「返信不能」と決めつけない。
    if (!talk || !stored || stored.from !== expectedFrom || !stored.inputEnabled || !talkAvailable(talk, state))
      return;
    // 全原本でruleを持たないblockは読み取り専用の終点。未取得の入力地点とは区別する。
    if (blocksById.get(expectedFrom)?.acceptsInput === false) return;
    if (!talk.rules.some(rule => rule.from === expectedFrom && rule.isDefault))
      throw new Error(`遷移先のdefaultが未取得です: ${talkId}/${expectedFrom}`);
  }
  function appAvailable(appId: string, state: StoredPlayerState) {
    const app = appById(appId);
    if (!app || app.unavailable || !conditionMet(app.cond, state))
      return false;
    return app.initialState === "normal" || state.repairedAppIds.includes(app.id);
  }
  function contentAvailable(content: ScenarioContent, state: StoredPlayerState) {
  ...
  function talkAvailable(talk: ScenarioTalk, state: StoredPlayerState) {
    if (talk.unavailable)
      return false;
    return talkDefinitionAvailable(talk, state) && talkRepaired(talk, state);
  }
  function chatAuthGateActive(state: StoredPlayerState) {
    return Boolean(workerScenario.chatAuthGate
      && conditionMet(workerScenario.chatAuthGate.cond, state)
      && appAvailable("chat", state));
  }
  function talkCanPost(talk: ScenarioTalk, state: StoredPlayerState) {
    if (!talkAvailable(talk, state) || (talk.kind === "chat" && chatAuthGateActive(state)))
      return false;
    if (state.talks[talk.id]?.inputEnabled === false)
      return false;
    const currentFrom = state.talks[talk.id]?.from;
    return Boolean(currentFrom && talk.rules.some((rule) => rule.from === currentFrom && rule.isDefault && conditionMet(rule.cond, state)));
  }
  function publicNotification(notificationId: string, state: StoredPlayerState) {
    ...
    if (!notification || !conditionMet(notification.cond, state))
      return null;
  ...
  function publicIncomingCall(internalId: string) {
    ...
    if (!call)
      return undefined;
    const media = resolveMediaRecord(call,workerScenario.attachments);
    return { id:call.publicId, name:call.name,
      ...(typeof media.audioUrl === "string" ? {audioUrl:media.audioUrl} : {}),
      ...(call.transcript ? {transcript:call.transcript} : {}) };
  }
  function visibleIncomingCallId(state: StoredPlayerState) {
    const id = state.incomingCallId;
    if (!id || state.completedIncomingCallIds.includes(id))
      return null;
  ...
  function publicTalkThread(talk: ScenarioDeviceTalk, state: StoredPlayerState) {
    ...
    const messages = initialTalkMessagesForState(talk, stored?.initialFormatEnv ?? {}, state.repairedContentIds).visibleMessages.map(publicTalkMessage);
    const shared = {
      id: talk.publicId,
      contentId: talk.publicId,
      messages,
  ...
  function visibleApps(state: StoredPlayerState) {
    return workerScenario.apps
      ...
      if (app.initialState === "hidden" && !available)
        return [];
      return [{
          id: app.id,
  ...
  function publicContentRecord(content: ScenarioContent) {
    const { unlockCode: _unlockCode, playbackCond: _playbackCond, playbackDisabledLabel: _playbackDisabledLabel, formDisabledCond: _formDisabledCond, ...sourceRecord } = content.record;
    const projectApp = projectAppById.get(content.appId);
    const projectedRecord = projectApp ? projectApp.publicRecord(sourceRecord)
      : usesStandardMedia(content.appId) ? resolveMediaRecord(sourceRecord,workerScenario.attachments) : sourceRecord;
    ...
    if (content.appId === "photos") {
      ...
      if (attachmentId)
        record.attachmentId = publicAttachmentId(attachmentId);
    }
    ...
    const audioSegments = Array.isArray(record.audioSegments)
      ? record.audioSegments.map((segment) => {
        if (!segment || typeof segment !== "object" || Array.isArray(segment))
          return segment;
        const item = segment as Record<string, unknown>;
        if (item.kind !== "generated" || typeof item.genAudioId !== "string")
          return item;
    ...
    return workerScenario.contents
      ...
      if (record.form && typeof record.form === "object" && !Array.isArray(record.form) && formDisabledCond) {
        record.form = {
          ...
          disabled: (record.form as {
            disabled?: unknown;
          }).disabled === true || conditionMet(formDisabledCond, state)
        };
      }
  ...
  function radioAudioCueForEvent(contentId: string, cueIndex: number, state: StoredPlayerState) {
  ...
  async function playerStateRevision(state: StoredPlayerState) {
  ...
  function publicMessageSegments(segments: readonly ScenarioMessageSegment[] | undefined) {
    return segments?.map((segment) => segment.kind === "link" && "contentId" in segment
      ? (() => {
  ...
  function publicSearchAgentTimelineItems(events: readonly StoredSearchAgentEvent[], talkId: string) {
  ...
  function materializedTalkMessagesForEvents(talk: ScenarioDeviceTalk, events: readonly StoredTalkEvent[]) {
    const ordered = [...events].sort((left, right) => (left.delivered_at.localeCompare(right.delivered_at) || left.id.localeCompare(right.id)));
  ...
  function visibleTalkMessagesForState(talk: ScenarioDeviceTalk, state: StoredPlayerState, events: readonly StoredTalkEvent[] = []) {
    const initial = initialTalkMessagesForState(talk, state.talks[talk.id]?.initialFormatEnv ?? {}, state.repairedContentIds).visibleMessages;
    return [...initial, ...materializedTalkMessagesForEvents(talk, events)];
  }
  async function publicPlayerState(state: StoredPlayerState, stateVersion: number, generatedAudio: PublicGeneratedAudioState[] = [], nextScenarioWakeAt: string | null = null, transcriptDeltas: readonly TranscriptAppend[] = []) {
    ...
    const listedTalks = workerScenario.talks.filter((talk): talk is ScenarioDeviceTalk => isDeviceTalk(talk) && (talkDefinitionAvailable(talk, state)
    ...
    for (const transcript of transcriptDeltas) {
      if (transcript.streamId === SEARCH_AGENT_STREAM_ID) {
        ...
        if (!talk || !isSearchAgentTalk(talk))
          continue;
        publicTranscriptDeltas.push({
          kind: "search_agent",
          talkId: talk.publicId,
          transcriptKey: transcript.transcriptKey,
          messages: publicSearchAgentTimelineItems(transcript.messages.filter((message): message is StoredSearchAgentEvent => message.kind === "search_agent"), talk.publicId)
        });
        continue;
      }
      if (!transcript.streamId.startsWith("talk:"))
        continue;
      const talk = talkByInternalId(transcript.streamId.slice("talk:".length));
      if (!talk || !talkAvailable(talk, state))
        continue;
      if (!isDeviceTalk(talk))
        continue;
      const resolvedMessages = transcript.resolvedMessages
        ?? materializedTalkMessagesForEvents(talk, transcript.messages.filter((message): message is StoredTalkEvent => "event_type" in message));
      publicTranscriptDeltas.push({
        kind: talk.kind,
        talkId: talk.publicId,
        transcriptKey: transcript.transcriptKey,
    ...
    const projectAppRecords = Object.fromEntries(workerScenario.projectAppIds.flatMap((appId) => {
      if (!workerScenario.apps.some((app) => app.id === appId))
        return [];
    ...
    return {
      ...
      projectState: Object.fromEntries(workerScenario.publicStateVariables.map((id) => [id, stateValues[id]])),
      visibleDeviceState: {
  ...
  function contentByPublicId(publicId: string) {
    return workerScenario.contents.find((content) => content.publicId === publicId) ?? null;
  }
  function contentByInternalId(internalId: string) {
  ...
  function visibleTalkAttachmentMatches(publicContentId: string, appId: string, state: StoredPlayerState) {
    ...
    if (!content || !conditionMet(content.cond, state) || !state.revealedAttachmentContentIds.includes(content.id))
      return false;
    return workerScenario.talks.some((talk) => {
      if (!isDeviceTalk(talk) || talk.appId !== appId || !talkAvailable(talk, state))
        return false;
      const stored = state.talks[talk.id];
      if (!stored)
        return false;
      ...
      }) => message.attachment?.contentId === content.id;
      if (visibleTalkMessagesForState(talk, state).some(matches))
        return true;
      return Object.entries(stored.blockDisplayCounts).some(([blockId, count]) => {
        if (!Number.isInteger(count) || count <= 0 || blocksById.get(blockId)?.talkId !== talk.id)
          return false;
  ...
  function observedAlbumMediaContentIds(talk: ScenarioTalk, state: StoredPlayerState, publicContentIds: readonly string[]) {
  ...
  function talkByInternalId(internalId: string) {
  ...
  function searchScenario(query: string, state: StoredPlayerState) {
    ...
    const contentResults = workerScenario.contents
      .filter((content) => (conditionMet(content.cond, state)
      ...
      const imageUrl = usesStandardMedia(content.appId) ? resolveMediaRecord(content.record,workerScenario.attachments,false).imageUrl : content.record.imageUrl;
      return {
        contentId: content.publicId,
        appId: content.appId,
  ...
  function repairTarget(publicContentId: string, appId: string) {
  ...
  function openTargetExists(publicContentId: string, appId: string, state: StoredPlayerState) {
    const app = appById(publicContentId);
    if (app?.id === appId)
      return conditionMet(app.cond, state);
    const content = contentByPublicId(publicContentId);
    if (content && typeof content.record.attachment === "string")
      return visibleTalkAttachmentMatches(publicContentId, appId, state);
    if (content && content.appId !== appId && visibleTalkAttachmentMatches(publicContentId, appId, state))
      return true;
  ...
  function notificationIdsForTarget(publicContentId: string, state: StoredPlayerState) {
  ...
  return {
    useScenario,
    validateTalkContinuation,
    validatePartState,
    talkEvents,
    get workerScenario() { return workerScenario; },
    isSearchAgentTalk,
    isDeviceTalk,
