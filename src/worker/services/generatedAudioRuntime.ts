...
export function createGeneratedAudioRuntime(runtime: Pick<ScenarioRuntime,"workerScenario">) {
  ...
  async function dispatchGeneratedAudioIntent(store: AppStore, playerId: string, job: GeneratedAudioJob) {
    ...
    return await store.updateGeneratedAudioJob(playerId, next) ? next : null;
  }
  async function reconcileGeneratedAudio(store: AppStore, playerId: string, existingJobs?: readonly GeneratedAudioJob[]) {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      if (!await store.updateGeneratedAudioJob(playerId, next)) { rows.splice(index,1); index -= 1; continue; }
      rows[index] = next;
    }
    return rows;
  }
  async function publicGeneratedAudioStates(store: AppStore, playerId: string, definitions = runtime.workerScenario.generatedAudio): Promise<PublicGeneratedAudioState[]> {
