...
import type { HookFlashEffectOptions, HookPresentationEffectOptions, ScenarioEventPayload, ScenarioHookContext } from "../../shared/hooks.ts";
import type { StoredTalkMessage } from "../../shared/scenario.ts";
import { MAX_SEARCH_AGENT_QUERY_LENGTH, SEARCH_AGENT_STREAM_ID, SEARCH_AGENT_TALK_ID } from "../../shared/searchAgent.ts";
import type { ScenarioHookHandlerRegistry } from "../../shared/hooks.ts";
import { copyStoredPlayerState, sha256, type StoredPlayerState, type StoredTalkEvent, type StoredTranscriptMessage, type TranscriptAppend } from "../../server/store.ts";
...
import { hookLlmModelVersion, hookLlmRequestKey, HookLlmUnavailableError, normalizeHookLlmMatchRequest, normalizeHookLlmRequest, resolveHookLlmRequest, type HookLlmRequest } from "./hookLlm.ts";
import { applyCompactStateAssignments, effectiveStateValues, setStateValue, stateValue } from "../stateValues.ts";
...
type RecordedEffect = {
  ...
  type: "incoming.markCompleted";
  callId: string;
} | {
  type: "incoming.clearActive";
} | {
  type: "content.setState";
  contentId: string;
  state: "repaired" | "unlocked";
  appId?: string;
} | {
  type: "app.repair";
  appId: string;
  ...
  talkId: string;
  blockId: string;
  formatEnv: Record<string, string>;
  mode: "advance" | "stay";
} | {
  type: "talk.search";
  talkId: typeof SEARCH_AGENT_TALK_ID;
  query: string;
} | {
  type: "talk.input";
  talkId: string;
  ...
  type: "genAudio.prepare";
  id: string;
  inputText: string;
} | {
  type: "presentation.effect";
  effect: ScenarioPresentationEffect;
} | {
  type: "presentation.sequence";
  sequence: ScenarioPresentationSequence;
};
// 定義はこの実行単位に閉じ込め、別プレイヤーの処理と共有変更しない。
export function createScenarioHooksRuntime(runtime: ScenarioRuntime, scenarioHookHandlers: ScenarioHookHandlerRegistry, SCENARIO_HOOK_TALK_BLOCKS: Readonly<Record<string, readonly string[]>>) {
  const { searchAgentResultEvent } = runtime.talkEvents;
  const {
  ...
  function targetForEvent(event: ScenarioEventPayload) {
    if (event.eventId === "part_loaded")
      return event.partId ?? "";
    if (event.eventId === "scheduled_event")
      return event.scheduleId ?? "";
    if (event.eventId === "audio_cue_reached")
      return event.cueTarget ?? "";
    if (event.eventId === "incoming_call_completed")
      return event.callId ?? "";
    if (event.eventId === "message_link_opened")
      return event.actionId ?? "";
    if (event.eventId === "talk_turn_completed")
      return event.talkId ?? "";
    if (event.eventId === "form_submitted")
      return event.formId ?? "";
  ...
  function cleanFields(fields: Record<string, string> = {}) {
    return Object.fromEntries(Object.entries(fields)
      ...
      .map(([key, value]) => [key.trim().slice(0, 64), value.slice(0, 1000)]));
  }
  const MAX_PRESENTATION_EFFECT_DURATION_MS = 8000;
  ...
  function capEffectTimings(fadeInMs: number, holdMs: number, fadeOutMs: number): Pick<ScenarioPresentationTransition, "fadeInMs" | "holdMs" | "fadeOutMs"> {
    const total = fadeInMs + holdMs + fadeOutMs;
    if (total <= MAX_PRESENTATION_EFFECT_DURATION_MS)
      return { fadeInMs, holdMs, fadeOutMs };
    // 合計だけを縮め、必須の両フェードへ最低1フレーム相当を残す。
    const scale = MAX_PRESENTATION_EFFECT_DURATION_MS / total;
    const normalizedFadeInMs = Math.min(MAX_PRESENTATION_EFFECT_DURATION_MS - MIN_PRESENTATION_FADE_DURATION_MS, Math.max(MIN_PRESENTATION_FADE_DURATION_MS, Math.round(fadeInMs * scale)));
    const normalizedFadeOutMs = Math.min(MAX_PRESENTATION_EFFECT_DURATION_MS - normalizedFadeInMs, Math.max(MIN_PRESENTATION_FADE_DURATION_MS, Math.round(fadeOutMs * scale)));
    return {
  ...
  function presentationTransition(options: HookPresentationEffectOptions | undefined, defaults: ScenarioPresentationTransition): ScenarioPresentationTransition {
    if (options !== undefined && (!options || typeof options !== "object" || Array.isArray(options))) {
      throw new Error("effect.flash / blackout のoptionsはobjectで指定してください。");
    }
    const input = options ?? {};
    const timings = capEffectTimings(presentationPhaseMs(input.fadeInMs, defaults.fadeInMs, MIN_PRESENTATION_FADE_DURATION_MS), presentationPhaseMs(input.holdMs, defaults.holdMs, 0), presentationPhaseMs(input.fadeOutMs, defaults.fadeOutMs, MIN_PRESENTATION_FADE_DURATION_MS));
  ...
  async function runScenarioHooks(state: StoredPlayerState, event: ScenarioEventPayload, services: {
    playerId?: string;
    ...
    const effectiveRecordedState = () => effectiveStateValues(runtime.workerScenario.stateVariables, recordedStateValues);
    ...
    if (!Number.isSafeInteger(maxRequests) || maxRequests < 1)
      throw new Error("LLM_HOOK_MAX_REQUESTSは1以上の整数にしてください。");
    ...
    const recordStateUpdates = (updates: readonly string[]) => {
      recordedStateValues = applyCompactStateAssignments(runtime.workerScenario.stateVariables, recordedStateValues, updates, {}, runtime.workerScenario.stateVariableDefinitions);
    ...
    const recordTalkBlock = (talkId: string, blockId: string, mode: "advance" | "stay") => {
      ...
      if (!allowedBlocks?.includes(blockId))
        throw new Error(`talk.addBlockの指定が不正です: ${talkId}/${blockId}`);
      const block = runtime.workerScenario.talkBlocks.find((item) => item.talkId === talkId && item.blockKey === blockId && !item.repeatOf);
      if (!block || block.unavailable)
        throw new Error(`talk.addBlockの本文が未取得または未定義です: ${talkId}/${blockId}`);
      const formatEnv = Object.fromEntries(Object.entries(effectiveRecordedState())
    ...
    const recordTalkInput = (talkId: string, action: "show" | "hide" | "enable" | "disable") => {
      if (!runtime.workerScenario.talks.some((talk) => talk.id === talkId)) {
    ...
    const assertPresentationEffectAllowed = (type: ScenarioPresentationEffect["type"]) => {
      if (event.eventId === "scheduled_event")
        throw new Error(`scheduled_eventではeffect.${type}を使用できません。`);
    };
    ...
    const endWithPresentationSequence = (sequence: ScenarioPresentationSequence): never => {
      if (event.eventId === "part_loaded")
        throw new Error("part_loadedではeffectSequenceを開始せず、後段の通常完了hookへ記述してください。");
      if (event.eventId === "scheduled_event")
        throw new Error(`scheduled_eventではeffectSequence.${sequence.type}を使用できません。`);
    ...
    const context: ScenarioHookContext = {
      state: {
        get(id) {
          if (!Object.prototype.hasOwnProperty.call(runtime.workerScenario.stateVariables, id))
            throw new Error(`状態変数が未取得または未定義です: ${id}`);
          return stateValue(runtime.workerScenario.stateVariables, recordedStateValues, id);
        },
        set(id, value) {
          recordedStateValues = setStateValue(runtime.workerScenario.stateVariables, recordedStateValues, id, value, runtime.workerScenario.stateVariableDefinitions);
      ...
      incoming: {
        start(callId) {
          if (!runtime.workerScenario.incomingCalls.some((call) => call.id === callId && !call.unavailable))
            throw new Error(`着信のpartが未取得または未定義です: ${callId}`);
          effects.push({ type: "incoming.start", callId });
        },
        markCompleted(callId) {
          if (!runtime.workerScenario.incomingCalls.some((call) => call.id === callId))
            throw new Error(`未定義のincomingCallです: ${callId}`);
      ...
      content: {
        setState(contentId, contentState, appId) {
          const content = runtime.workerScenario.contents.find((item) => item.id === contentId);
          const talk = runtime.workerScenario.talks.find((item) => item.id === contentId && !isSearchAgentTalk(item) && item.initialState !== "normal");
          if ((!content && !talk) || content?.unavailable || talk?.unavailable)
            throw new Error(`contentのpartが未取得または未定義です: ${contentId}`);
      ...
      app: {
        repair(appId) {
          if (!runtime.workerScenario.apps.some((app) => app.id === appId && !app.unavailable))
            throw new Error(`appのpartが未取得または未定義です: ${appId}`);
      ...
      talk: {
        addBlock(talkId, blockId, options = {}) {
          ...
          if (mode !== "advance" && mode !== "stay")
            throw new Error(`talk.addBlockのmodeが不正です: ${String(mode)}`);
        ...
        search(talkId: typeof SEARCH_AGENT_TALK_ID, query: string) {
          ...
          if (talkId !== SEARCH_AGENT_TALK_ID || !normalizedQuery)
            throw new Error("talk.searchの指定が不正です。");
      ...
      todo: {
        add(todoId) {
          if (!runtime.workerScenario.todos.some((todo) => todo.id === todoId))
            throw new Error(`未定義のtodoです: ${todoId}`);
      ...
      schedule: {
        after(scheduleId, delayMs, fields = {}, instanceId = scheduleId) {
          if (!scheduleId.trim() || !instanceId.trim() || !Number.isFinite(delayMs) || delayMs < 0)
            throw new Error("schedule.afterの指定が不正です。");
          if (!runtime.workerScenario.hooks.some((hook) => hook.event === "scheduled_event" && hook.target === scheduleId.trim())) {
          ...
          effects.push({
            ...
            delayMs: Math.min(Math.round(delayMs), 365 * 24 * 60 * 60 * 1000),
            fields: cleanFields(fields)
          });
        },
        cancel(instanceId) {
          if (!instanceId.trim())
            throw new Error("schedule.cancelの指定が不正です。");
          claimEffectResource("schedule", instanceId.trim());
      ...
      genAudio: {
        prepare(id, options) {
          if (!runtime.workerScenario.generatedAudio.some((audio) => audio.id === id) || !options.inputText.trim())
      ...
      effectSequence: {
        ...
        allClear(appId, contentId, autoplay = true) {
          if (!runtime.workerScenario.apps.some((app) => app.id === appId) || !runtime.workerScenario.contents.some((content) => content.id === contentId && content.appId === appId && !content.unavailable)) {
    ...
    const eligibleHooks = runtime.workerScenario.hooks.filter((hook) => (hook.event === event.eventId
      && (!hook.target || hook.target === "*" || hook.target === target)));
    try {
      for (const hook of eligibleHooks) {
        if (!evaluateCondition(hook.cond, definedConditionState(effectiveRecordedState())))
          continue;
        const handler = scenarioHookHandlers[hook.handler as keyof typeof scenarioHookHandlers];
        if (!handler)
          throw new Error(`hook handlerが登録されていません: ${hook.handler}`);
        const returned = (handler as (context: ScenarioHookContext, event: ScenarioEventPayload) => unknown)(context, event);
        if (returned && typeof (returned as {
          then?: unknown;
        }).then === "function")
          throw new Error(`hook handlerは同期関数にしてください: ${hook.handler}`);
      }
    }
    catch (error) {
      if (error instanceof ScenarioHookPublicRejection)
        return publicRejectionResult(state, error);
      if (error instanceof ScenarioHookSequenceEnd) {
        ...
        if (llmResults.size >= maxRequests)
          throw new Error("too_many_llm_hook_requests");
    ...
    async function ensureTalk(talkId: string) {
      const definition = runtime.workerScenario.talks.find((talk) => talk.id === talkId);
      if (!definition || !talkAvailable(definition, nextState))
        return null;
      if (!nextState.talks[talkId]) {
        if (isSearchAgentTalk(definition)) {
          ...
          const revealed = revealTalkMessages(nextState, talkId, initial.messages.filter((message) => message.type === "message"));
        ...
        const initial = initializeTalkState(definition, turnKey, effectiveStateValues(runtime.workerScenario.stateVariables, nextState.stateValues), nextState.repairedContentIds);
        nextState.talks[talkId] = initial.state;
    ...
    function appendTranscript(talkId: string, events: StoredTranscriptMessage[], messages: StoredTalkMessage[] = []) {
      ...
      if (current) {
        ...
        if (messages.length)
          current.resolvedMessages = [...(current.resolvedMessages ?? []), ...messages];
      }
      else {
    ...
    function repairContent(contentId: string) {
      ...
      const talk = runtime.workerScenario.talks.find((item) => item.id === contentId && !isSearchAgentTalk(item) && item.initialState !== "normal");
      if (!content && !talk)
        throw new Error(`未定義のcontentです: ${contentId}`);
      if (nextState.repairedContentIds.includes(contentId))
        return;
      const restored = content ? restoredTalkHistoryMessages(nextState, contentId) : null;
      if (restored && !restored.ok)
        throw new Error(`talk初期履歴を復元できません: ${contentId}/${restored.error}`);
      ...
      if (talk)
        synchronizeInitialTalkLastOtherMessageId(nextState, talk.id);
    }
    for (const effect of effects) {
      if (effect.type === "state.set") {
        nextState.stateValues = setStateValue(runtime.workerScenario.stateVariables, nextState.stateValues, effect.id, effect.value, runtime.workerScenario.stateVariableDefinitions);
      }
      else if (effect.type === "state.apply") {
        nextState.stateValues = applyCompactStateAssignments(runtime.workerScenario.stateVariables, nextState.stateValues, effect.updates, {}, runtime.workerScenario.stateVariableDefinitions);
      }
      else if (effect.type === "incoming.start") {
        if (!nextState.completedIncomingCallIds.includes(effect.callId))
          nextState.incomingCallId = effect.callId;
      }
        ...
        if (nextState.incomingCallId === effect.callId)
        ...
        if (effect.state === "unlocked")
          nextState.unlockedContentIds = appendUnique(nextState.unlockedContentIds, effect.contentId);
      }
        ...
        if (!current)
          throw new Error(`hookのtalk.${effect.action}Inputを適用できません: ${effect.talkId}`);
        ...
        const current = await ensureTalk(effect.talkId);
        if (!current)
          throw new Error(`hookのtalk操作を適用できません: ${effect.talkId}`);
        ...
        if (isSearchAgentTalk(current.definition)) {
          ...
          const revealed = revealTalkMessages(nextState, effect.talkId, rendered.messages.filter((message) => message.type === "message"));
          nextState.revealedMessageLinks = revealed.revealedMessageLinks;
        ...
        const current = await ensureTalk(effect.talkId);
        if (!current || !isSearchAgentTalk(current.definition))
          throw new Error(`hookのtalk.searchを適用できません: ${effect.talkId}`);
