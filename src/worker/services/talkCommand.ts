import type { StoredPlayerState } from "../repository.ts";
import { contentAvailable, contentByInternalId, contentByPublicId } from "../scenario.ts";
...
export function talkCommandAvailable(value: string, state: StoredPlayerState) {
  ...
  return command.kind === "photo"
    ? typeof content.record.imageUrl === "string" || typeof content.record.audioUrl === "string"
    : true;
}
